...
class LMFYoloV2(YoloV2):
    ...
    def base(self, images, is_training):
        ...
        def connect_block(lateral, up, name):
            with tf.variable_scope(name_or_scope=name):
                up = tf.depth_to_space(up, block_size=2)
                up = darknet_block("block", up, filters=64, kernel_size=1)
                out = tf.concat([lateral, up], axis=concat_axis)
            return out
        ...
        x4 = darknet_block("block_4", x, filters=32, kernel_size=3)
        x = self._reorg("pool_3", x4, stride=2, data_format=self.data_format)
        c1 = darknet_block("c1", x, filters=64, kernel_size=1)

        x5 = darknet_block("block_5", x, filters=64, kernel_size=3)
        x = self._reorg("pool_4", x5, stride=2, data_format=self.data_format)
        c2 = darknet_block("c2", x, filters=64, kernel_size=1)
...
class LMFYoloV2Quantize(LMFYoloV2):
...
class LMFYoloV2_2(LMFYolo):
    ...
    def base(self, images, is_training):
        ...
        x = darknet_block("block_9", merged, filters=256, kernel_size=3)
        x = darknet_block("block_10", x, filters=256, kernel_size=3)
        x11 = darknet_block("block_11", x, filters=256, kernel_size=3)

        x = tf.depth_to_space(x11, block_size=2)
        x = darknet_block("block_12", x, filters=64, kernel_size=3)
        x6_2 = darknet_block("block_6_1x1_2", x6, filters=64, kernel_size=1)
        x = tf.concat([x6_2, x], axis=concat_axis, name="connect_block_1")
        x = darknet_block("block_13", x, filters=128, kernel_size=1)

        x = tf.depth_to_space(x, block_size=2)
        x = darknet_block("block_14", x, filters=32, kernel_size=3)
        x5_2 = darknet_block("block_5_1x1_2", x5, filters=32, kernel_size=1)
        x11_d2s = tf.depth_to_space(x11, block_size=4)
        x = tf.concat([x5_2, x11_d2s, x], axis=concat_axis, name="connect_block_2")
        x = darknet_block("block_15", x, filters=64, kernel_size=1)

        x = darknet_block("block_16", x, filters=64, kernel_size=3, activation=self.before_last_activation)
...
class LMFYoloV2_2Quantize(LMFYoloV2_2):
...
class LMFYoloV2_3(LMFYolo):
    ...
    def base(self, images, is_training):
        ...
        x4_1 = darknet_block("block_4_1x1", x4, filters=16, kernel_size=1)
        x5_1 = darknet_block("block_5_1x1", x5, filters=32, kernel_size=1)

        x4_s2d = self._reorg("block_4_s2d", x4_1, stride=4, data_format=self.data_format)
        x5_s2d = self._reorg("block_5_s2d", x5_1, stride=2, data_format=self.data_format)

        x7 = darknet_block("block_6", x, filters=128, kernel_size=3)
        x8 = darknet_block("block_7", x7, filters=256, kernel_size=3)

        merged = tf.concat([x8, x7, x5_s2d, x4_s2d],
                           axis=concat_axis, name="block_merged")

        x = darknet_block("block_8", merged, filters=256, kernel_size=3)
        x = darknet_block("block_9", x, filters=256, kernel_size=3)
        x11 = darknet_block("block_10", x, filters=256, kernel_size=3)

        x = tf.depth_to_space(x11, block_size=2)
        x = darknet_block("block_11", x, filters=64, kernel_size=3)
        x5_2 = darknet_block("block_5_1x1_2", x5, filters=32, kernel_size=1)
        x = tf.concat([x5_2, x], axis=concat_axis, name="connect_block_2")
        x = darknet_block("block_12", x, filters=128, kernel_size=1)

        x = tf.depth_to_space(x, block_size=2)
        x = darknet_block("block_13", x, filters=32, kernel_size=3)
        x4_2 = darknet_block("block_4_1x1_2", x4, filters=32, kernel_size=1)
        x = tf.concat([x4_2, x], axis=concat_axis, name="connect_block_2")
        x = darknet_block("block_14", x, filters=64, kernel_size=1)

        x = darknet_block("block_15", x, filters=64, kernel_size=3, activation=self.before_last_activation)
...
class LMFYoloV2_3Quantize(LMFYoloV2_3):
