...
namespace p_1x3 = conv1x3_params;
...
void qconv1x3_impl(T_q in_data[], T_out out_data[], T_q k_data[], T_out threshold_data[],
                   unsigned in_w, unsigned in_h, unsigned in_c_by_word, unsigned in_b,
                   unsigned out_w, unsigned out_h, unsigned out_c,
                   unsigned pad_w, unsigned pad_h,unsigned stride);

template <int KH, int KW>
void conv(T_in in_data[], T_out out_data[], T_k k_data[], T_out threshold_data[],
          unsigned in_w, unsigned in_h, unsigned in_c,
          unsigned out_w, unsigned out_h, unsigned out_c,
          unsigned pad_w, unsigned pad_h, unsigned stride)
{
  ...
  /* } else { */
    cpp::conv1x3_impl(in_data, out_data, k_data, threshold_data, in_w, in_h, in_c, out_w, out_h, out_c, pad_w, pad_h, stride);
   /* } */
}
/// @}
...
template <int KH, int KW>
void qconv(T_q in_data_packed[], T_out out_data[], T_q k_data_packed[],  T_out threshold_data[],
           ...
           unsigned pad_w, unsigned pad_h, unsigned stride)
{
  ...
  /* } else { */
  /*   // cpp::qconv1x3_impl(in_data_packed, out_data, k_data_packed, in_w, in_h, in_c_by_word, out_w, out_h, out_c, 0, pad, */
  /* } */

  cpp::qconv1x3_impl(in_data_packed, out_data, k_data_packed, threshold_data,
                     in_w, in_h, in_c_by_word, nbits_in_data,
                     out_w, out_h, out_c,
                     pad_w, pad_h, stride);
                        ...
                        const unsigned out_c, const unsigned pad_w, const unsigned pad_h,  const unsigned stride)
{
  ...
  cpp::conv_kn2row_tiling_impl(in_data, out_data, k_data, threshold_data,
                               in_w, in_h, in_c,
                               out_w, out_h, out_c,
                               KW, KH,
                               pad_w, pad_h, stride);
                         ...
                         const unsigned pad_w, const unsigned pad_h, const unsigned stride)
{
  ...
  qconv_kn2row_tiling_impl(in_data_packed, out_data, k_data_packed, threshold_data,
                           in_w, in_h, in_c_by_word, in_b,
                           out_w, out_h, out_c,
                           KW, KH,
                           pad_w, pad_h);
}
